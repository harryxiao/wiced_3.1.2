...
download_bootloader: bootloader display_map_summary sflash_write_app $(WIPE_DEPENDENCY)
	...
	$(call CONV_SLASHES,$(OPENOCD_FULL_NAME)) -f $(OPENOCD_PATH)$(JTAG).cfg -f $(OPENOCD_PATH)$(HOST_OPENOCD).cfg -f apps/waf/sflash_write/sflash_write.tcl -c "sflash_write_file $(BOOTLOADER_EPM_FILE) $(SFLASH_BOOTLOADER_LOC) $(subst /,_,$(PLATFORM_FULL)-$(BUS)) 1 1" -c shutdown #$(DOWNLOAD_LOG) 2>&1
...
wipe_all:display_map_summary sflash_write_app
	...
	$(QUIET)$(call CONV_SLASHES,$(OPENOCD_FULL_NAME)) -f $(OPENOCD_PATH)$(JTAG).cfg -f $(OPENOCD_PATH)$(HOST_OPENOCD).cfg -f apps/waf/sflash_write/sflash_write.tcl -c "sflash_erase $(subst /,_,$(PLATFORM_FULL)-$(BUS)) 1" -c shutdown $(DOWNLOAD_LOG) 2>&1
