...
static void           format_wep_keys  ( char* wep_key_ouput, const char* wep_key_data, uint8_t* wep_key_length, wep_key_format_t wep_key_format );
...
static void format_wep_keys( char* wep_key_ouput, const char* wep_key_data, uint8_t* wep_key_length, wep_key_format_t wep_key_format )
{
    ...
    if ( wep_key_format == WEP_KEY_HEX_FORMAT )
    {
        ...
        for ( a = 0; a < wep_key[0].length; ++a )
        {
            uint8_t nibble1;
            uint8_t nibble2;
            hexchar_to_nibble( wep_key_data[a*2],     &nibble1 );
            hexchar_to_nibble( wep_key_data[a*2 + 1], &nibble2 );
            wep_key[0].data[a] = ( nibble1 << 4 ) | nibble2;
        }
    }
    else
    {
    ...
    uint8_t wep_key_entry_size = 2 + *wep_key_length;
    ...
    *wep_key_length = 4 * wep_key_entry_size;
}
